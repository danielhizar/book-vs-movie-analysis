...
#create a mutate to add a column called "paragraph" to both data frames that contain paragraph number
...
emma_corpora <- bind_rows(emma_book, emma_script) %>% 
  rename(text = ".")
...
# count how many of each trigram per corpus
emma_trigrams %>% 
  filter(!is.na(ngrams)) %>% 
